...
"FIXME: only a single fjCapability is highlighted, nextgroup= not working as expected?
...
"FIXME: only a single fjProtocol is highlighted, nextgroup= not working as expected?
syn keyword fjProtocol unix inet inet6 netlink packet contained nextgroup=fjProtocol
" Grabbed from: https://filippo.io/linux-syscall-table/
"FIXME: only a single fjSyscall is highlighted, nextgroup= not working as expected?
...
syn match fjCommand /\v^(include|noblacklist|nowhitelist|ignore|quiet|blacklist(-nolog)?|bind|disable-mnt|keep-var-tmp|mkdir|mkfile|noexec|overlay(-named|-tmpfs)?|private(-home|-cache|-bin|-dev|-etc|-lib|-opt|-srv|-tmp)?|keep-dev-shm|read-only|read-write|tmpfs|whitelist|writable-etc|writable-run-user|writable-var(-log)?|apparmor|caps|seccomp.block-secondary|memory-deny-write-execute|nonewprivs|noroot|xephyr-screen|rlimit-as|rlimit-cpu|rlimit-fsize|rlimit-nproc|rlimit-nofile|rlimit-sigpending|cpu|nice|cgroup|timeout|allusers|name|nodvd|nogroups|ipc-namespace|nodbus|nosound|noautopulse|notv|nou2f|novideo|no3d|defaultgw|dns|hostname|hosts-file|ip(6|range)?|mac(hine-id)?|mtu|net(filter|mask)?|veth-name|join-or-start)/
